...
import {
    makeCircle, makeRectangle, makeEquiTriangle,
    makeBeside, makeAbove, makeOverlay,
    makePlace, emptyScene, makeColor,
    paint, makeText, makeRotate
} from './image.js';
...
import { takeKey } from './components/App';
...
const protoEnv = [
    ...
    {
        name: 'number->string', binding: {
            type: RFUNCT_T,
            value: numberToString
        }
    },
    {
        name: 'string-length', binding: {
            type: RFUNCT_T,
            value: stringLength
        }
    },
    {
        name: 'string-append', binding: {
            type: RFUNCT_T,
            value: stringAppend
        }
    },
    {
        name: 'substring', binding: {
            type: RFUNCT_T,
            value: substring
        }
    },
    ...
    {
        name: 'overlay/align', binding: {
            type: RFUNCT_T,
            value: overlayAlign
        }
    },
    {
        name: 'place-image', binding: {
            type: RFUNCT_T,
            value: placeImage
        }
    },
...
function makeStruct(superID, fieldIDs, env) {
    ...
    function isID(args) {
        ...
        return {
            value: struct.type === RSTRUCT_T && struct.value.id === superID,
            type: RBOOL_T
        };
    }
    ...
    let extedEnv = [{
        name: `make-${superID}`, binding: {
            type: RFUNCT_T,
            value: construct
        }
    },
    {
        name: `${superID}?`, binding: {
            type: RFUNCT_T,
            value: isID
        }
    },
    ...fieldExtractors.map((extract, i) => (
        {
            name: `${superID}-${fieldIDs[i]}`, binding: {
                type: RFUNCT_T,
                value: extract
            }
        }
...
function isnull(args) {
    ...
    return {
        value: firstArg.value === null && firstArg.type === RLIST_T,
        type: RBOOL_T
    };
}
...
function stringcontains(args) {
    ...
    return {
        value: sndArg.value.includes(firstArg.value),
        type: RBOOL_T
    };
}
...
function substring(args) {
    ...
    return {
        value: firstArg.value.slice(sndArg.value, thrArg.value),
        type: RSTRING_T
    };
}
...
export {
    interp, initEnv, parsePrefix, interpPrefix,
